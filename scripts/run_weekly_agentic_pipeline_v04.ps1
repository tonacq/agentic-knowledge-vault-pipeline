<#
Script: run_weekly_agentic_pipeline_v04.ps1
Version: 04
Status: Candidate - Claude authentication diagnostics
...
Requires:
  ...
  - claude CLI authenticated for the account running this script
#>
...
function Get-ClaudeAuthenticationRemediation {
    param([string]$Details)

    $message = @(
        "Claude CLI authentication is unavailable or expired.",
        "Run these commands interactively as the SAME Windows account that runs the pipeline:",
        "",
        "    claude auth login",
        "    'AUTH_OK' | claude -p",
        "",
        "Then rerun the pipeline.",
        "For unattended Scheduled Task execution, the task must run under that same authenticated account."
    )

    if (-not [string]::IsNullOrWhiteSpace($Details)) {
        $message += @("", "Claude detail: $Details")
    }
...
function Test-ClaudeAuthentication {
    ...
    try {
        $authOutput = @(& claude auth status 2>&1)
        $authExit = $LASTEXITCODE
    } catch {
        return
    }

    $authText = ($authOutput | ForEach-Object { [string]$_ }) -join " "

    if (
        $authExit -ne 0 -or
        $authText -match '(?i)(expired|not authenticated|not logged in|failed to authenticate)'
    ) {
        throw (Get-ClaudeAuthenticationRemediation -Details $authText)
    }
}

function Get-ClaudeInvocationError {
    param(
        [AllowNull()]$ClaudeResult,
        [int]$ExitCode,
        [string]$OutputPath
    )

    $raw = (($ClaudeResult | ForEach-Object { [string]$_ }) -join [Environment]::NewLine).Trim()
    $detail = $raw

    if (-not [string]::IsNullOrWhiteSpace($raw)) {
        try {
            $parsed = $raw | ConvertFrom-Json -ErrorAction Stop
            if ($parsed.result) { $detail = [string]$parsed.result }
        } catch {
            # Preserve raw output when Claude did not return valid JSON.
        }
    }

    if ($detail -match '(?i)(oauth session expired|failed to authenticate|authentication.*expired)') {
        return Get-ClaudeAuthenticationRemediation -Details $detail
    }

    return (
        "Claude synthesis failed with exit code $ExitCode. " +
        "See $OutputPath" +
        $(if ($detail) { [Environment]::NewLine + "Claude detail: $detail" } else { "" })
    )
}
...
try {
    ...
    if (-not $synthesisRequired) {
        ...
        foreach ($currentPromptPath in $promptPaths) {
            ...
            if ($claudeExit -ne 0) {
                throw (Get-ClaudeInvocationError `
                    -ClaudeResult $claudeResult `
                    -ExitCode $claudeExit `
                    -OutputPath $currentClaudeOutputPath)
            }
